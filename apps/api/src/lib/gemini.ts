...
[첨부 자료 사용 규칙]
- 첨부 문서(추출된 텍스트)·이미지가 함께 주어진 경우, 그 안에서 명시적으로 도출되는 사실만 STAR에 사용한다.
- 추출 텍스트가 모호하거나 끊겨 있으면 그 부분을 무리하게 보간하지 말고 "구체적 내용 미기재"로 둔다.
- 첨부 자료에 적힌 수치·결과·인물·기관도 입력 텍스트와 마찬가지로 그대로만 인용하며, 새로 만들어내지 말 것.
- 이미지는 보조 증빙으로만 활용. 이미지에서 읽히지 않는 정보를 추측하지 말 것.
...
function buildSingleUserPrompt(input: SingleAnalysisInput): string {
  ...
  // 문서 추출 텍스트 — 비어있는 항목은 plan에 따라 통째로 제외
  const docs = (input.documentTexts ?? []).filter((d) => d.text.trim().length > 0);
  if (docs.length > 0) {
    lines.push("");
    lines.push("[첨부 문서 (학생 업로드 자료에서 추출·정제한 텍스트)]");
    docs.forEach((d, i) => {
  ...
  // 이미지는 prompt에 메타 표기만 — 실제 데이터는 inlineData로 따로 전달
...
export async function analyzeSingleActivity(input: SingleAnalysisInput): Promise<StarItem> {
  ...
  // parts: [텍스트 prompt, ...이미지 inlineData들]
