...
export type SingleAnalysisInput = {
  ...
  // DOCX 등 텍스트 추출 포맷 — prompt에 인라인으로 포함.
  documentTexts?: { filename: string; kind: string; text: string; truncated: boolean }[];
  // PDF 원본 — Gemini에 inlineData(application/pdf, base64)로 그대로 전달.
  pdfs?: { filename: string; mimeType: "application/pdf"; base64: string }[];
  // JPG/PNG 원본 — inlineData로 그대로 전달.
...
[첨부 자료 사용 규칙]
- 첨부 자료는 두 가지 형태로 주어진다.
  (a) DOCX 추출 텍스트 — prompt에 인라인으로 포함됨. 본문 일부만 포함될 수 있으며 끊긴 부분이 있을 수 있다.
  (b) PDF·이미지 원본 — inlineData로 직접 모델이 본다. 표·도해·서명·다이어그램 등 시각 정보를 그대로 활용 가능.
- (a)·(b) 모두에서 명시적으로 확인되는 사실만 STAR에 사용하라. 보이지 않는 정보를 추론·환각하지 말 것.
- PDF·이미지에서 시각적으로 읽히는 텍스트·표·도해의 내용을 그대로 인용하되, 흐릿하거나 부분만 보이는 경우 무리한 보간 금지.
- 추출 텍스트(DOCX)가 모호하거나 끊겨 있으면 "구체적 내용 미기재"로 둔다.
- 첨부 자료에 적힌 수치·결과·인물·기관도 입력 텍스트와 마찬가지로 그대로만 인용하며, 새로 만들어내지 말 것.
...
function buildSingleUserPrompt(input: SingleAnalysisInput): string {
  ...
  // DOCX 추출 텍스트 — 비어있는 항목은 통째로 제외
  const docs = (input.documentTexts ?? []).filter((d) => d.text.trim().length > 0);
  if (docs.length > 0) {
    lines.push("");
    lines.push("[첨부 문서 (DOCX에서 추출·정제한 텍스트)]");
    docs.forEach((d, i) => {
      const label = `${d.filename} (${d.kind}${d.truncated ? ", 일부 생략됨" : ""})`;
      lines.push("");
  ...
  // PDF는 prompt에 메타 표기만 — 실제 데이터는 inlineData로 따로 전달
  if (input.pdfs && input.pdfs.length > 0) {
    lines.push("");
    lines.push("[첨부 PDF (원본)]");
    lines.push(
      "이 메시지에 함께 첨부된 PDF를 직접 읽어 시각 정보(표·도해·서명 포함)를 활용하라. " +
        "보이지 않는 정보를 추측하지 말 것."
    );
    input.pdfs.forEach((p, i) => {
      lines.push(`- PDF ${i + 1}: ${p.filename}`);
    });
  }

  // 이미지도 prompt에 메타 표기만 — 실제 데이터는 inlineData로 따로 전달
  if (input.images && input.images.length > 0) {
    lines.push("");
    lines.push("[첨부 이미지 (원본)]");
    lines.push(
      "이 메시지에 함께 첨부된 이미지를 보조 증빙 자료로 참고하라. " +
        "이미지에서 명확히 읽히는 정보만 사용하고, 그 외는 추측하지 말 것."
    );
    input.images.forEach((img, i) => {
      lines.push(`- 이미지 ${i + 1}: ${img.filename} (${img.mimeType})`);
    });
  }

  lines.push("");
  lines.push(
    "위 활동(과 첨부 자료가 있다면 그 내용도)을 STAR 5요소로 정제하고 활동의 핵심 키워드 3~5개를 추출해. activity_index 는 입력 그대로 반환할 것."
  );
  return lines.join("\n");
}
...
export async function analyzeSingleActivity(input: SingleAnalysisInput): Promise<StarItem> {
  ...
  // parts: [텍스트 prompt, ...PDF inlineData들, ...이미지 inlineData들]
  const parts: Array<{ text: string } | { inlineData: { mimeType: string; data: string } }> = [
    { text: buildSingleUserPrompt(input) },
  ];
  for (const pdf of input.pdfs ?? []) {
    parts.push({ inlineData: { mimeType: pdf.mimeType, data: pdf.base64 } });
  }
