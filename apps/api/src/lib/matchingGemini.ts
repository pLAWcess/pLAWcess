...
4순위 — 정성 유사도. 다음을 모두 종합 판단:
  - ai_keywords (KeywordCount[]) — 통합 분석 결과. 없을 수도 있음.
  - ai_story_outline — 통합 분석 결과 자소서 흐름. 없을 수도 있음.
  - star_analysis_brief — 활동별 STAR 분석에서 summary + keywords 만 발췌. 통합 분석 안 했어도 채워질 수 있음.
  - core_keywords / career_goal / 멘토 personal_statement_summary / 멘티 storySummary.
  - 키워드 교집합·활동 흐름·진로 정합성을 종합. star_analysis_brief 만으로도 충분히 비교 가능.
...
- 정성 줄 "데이터 부족" 표기 조건: 멘티 또는 멘토 한쪽이라도 ai_keywords, ai_story_outline, star_analysis_brief 가 **모두** 비어있을 때만.
...
function buildUserPrompt(
  ...
  const lines: string[] = [
    ...
    `- 자소서 요약(storySummary): ${mentee.storySummary ?? "(없음)"}`,
    `- AI 정성 키워드(ai_keywords): ${safeJson(mentee.aiKeywords)}`,
    `- AI 자소서 흐름(ai_story_outline): ${safeJson(mentee.aiStoryOutline)}`,
  ...
  mentors.forEach((m, i) => {
    ...
    lines.push(`- 사용자 입력 키워드: ${m.coreKeywords ?? "(없음)"}`);
    lines.push(`- 자소서 요약(personal_statement_summary): ${m.personalStatementSummary ?? "(없음)"}`);
    lines.push(`- AI 정성 키워드(ai_keywords): ${safeJson(m.aiKeywords)}`);
    lines.push(`- AI 자소서 흐름(ai_story_outline): ${safeJson(m.aiStoryOutline)}`);
