...
export default function SelectField({ value, options, onChange, placeholder }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <ul className="absolute z-10 left-0 mt-1 min-w-full w-max bg-white border border-border rounded-lg shadow-md py-1 text-sm text-text-primary max-h-60 overflow-y-auto">
          {options.map((opt) => (
            <li key={opt}>
              <button
                type="button"
